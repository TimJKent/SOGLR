...
// This example has nearly identical code to 2 with the only exception being the shaders used.

int main()
{
    // Render maintains its own window
    SOGLR::Renderer renderer;
    renderer.GetWindow()->SetTitle("3_RainbowTriangle");
    ...
    SOGLR::Shader rainbow_triangle_shader(vert_file, frag_file);

    while (renderer.IsRunning())
    {
        renderer.BeginFrame();
        rainbow_triangle_shader.Bind();
